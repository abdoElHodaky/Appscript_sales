/**
 * ============================================================
 * Entry Point — Code.gs  [v5.1]
 * ------------------------------------------------------------
 * Changes: AppSheet sync triggers + wireAppSheetSync
 * ============================================================
 */
...
function initializeSystem() {
  ...
  // 2. Create AppSheet view sheets
  const syncSvc = c.getAppSheetSyncService();
  syncSvc.getOrCreateSheet_('app_orders', AppSheetSyncService.ORDER_HEADERS);
  syncSvc.getOrCreateSheet_('app_customers', AppSheetSyncService.CUSTOMER_HEADERS);

  // 3. Wire outbound AppSheet sync listeners
  c.wireAppSheetSync();

  // 4. Install triggers (remove duplicates first)
  const triggers = ScriptApp.getProjectTriggers();
  for (let i = 0; i < triggers.length; i++) {
    const fn = triggers[i].getHandlerFunction();
    if (fn === 'runAlertEvaluation' || fn === 'runAppSheetSync') {
      ScriptApp.deleteTrigger(triggers[i]);
    }
  }
  ScriptApp.newTrigger('runAlertEvaluation').timeBased().everyHours(1).create();
  ScriptApp.newTrigger('runAppSheetSync').timeBased().everyMinutes(5).create();
...
function runAppSheetSync() {
  return container().getSyncOrdersFromAppSheetUseCase().execute();
}
